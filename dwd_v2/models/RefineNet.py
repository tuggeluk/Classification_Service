...
def build_refinenet(inputs, num_classes= None, preset_model='RefineNet-Res101', weight_decay=1e-5, is_training=True, upscaling_method="bilinear", pretrained_dir="models",substract_mean = True,
    ...
    if preset_model == 'RefineNet-Res50':
        with slim.arg_scope(resnet_arg_scope(weight_decay=weight_decay)):
            logits, end_points = resnet_v1_50(inputs, is_training=is_training, scope='resnet_v1_50')
        ...
        with slim.arg_scope(resnet_arg_scope(weight_decay=weight_decay)):
            logits, end_points = resnet_v1_101(inputs, is_training=is_training, scope='resnet_v1_101')
            # RefineNet requires pre-trained ResNet weights
        ...
        with slim.arg_scope(resnet_arg_scope(weight_decay=weight_decay)):
            logits, end_points = resnet_v1_152(inputs, is_training=is_training, scope='resnet_v1_152')
            # RefineNet requires pre-trained ResNet weights
